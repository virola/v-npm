...
function getOssPolicy(dir) {
  return new Promise((resolve, reject) => {
    request({ url: '/common/getOssPolicy?dir=' + dir })
      .then(function(res) {
        resolve(res)
      })
      .catch(err => {
 ...
 */
function doData(params, file, compress = false) {
  //图片压缩处理
  return new Promise((resolve, reject) => {
 ...
 */
const upload = async (files, dir, callback, compress = true) => {
  const succFiles = []
  ...
  if (resp.success) {
    ...
    for (let i = 0; i < files.length; i++) {
      let formData = await doData(params, files[i], compress)
