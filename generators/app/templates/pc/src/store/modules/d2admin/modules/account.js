  ...
  actions: {
        ...
        $api.AccountLogin(data)
          .then(async res => {
            if (res.code == 200) {
              ...
              // 登录之后拉取用户数据
              const userData = await dispatch('getLoginInfo')
              if (userData) {
                resolve(userData)
              }
            } else {
    ...
    getLoginInfo({ dispatch, commit }) {
      return new Promise(async (resolve, reject) => {
        $api.getLoginInfo().then(async res => {
          if (res.code == 200) {
            console.log(res)
            ...
            // 设置角色
            // 根据实际业务情形，结合后端接口设定框架级用户角色，
            // 目前支持根据不同角色控制侧边菜单栏的显示
            const roles = res.roles
            await dispatch('d2admin/user/setRole', roles.map(role => setting.role.map[role]), { root: true })
            // 用户登录后从持久化数据加载一系列的设置
            await dispatch('load')
