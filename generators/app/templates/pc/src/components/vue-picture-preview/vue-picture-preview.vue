...
<transition name="fade" @after-leave="afterLeaveTransition">
  <div class="lg-preview-wrapper" v-show="preview.show" @click="leave" @touchmove.prevent>
    ...
    <div class="lg-preview-nav-left" v-if="preview.isHorizontalNavEnable" v-show="!preview.loading" @click="preAction">
      <span class="lg-preview-nav-arrow"></span>
    </div>
    <div class="lg-preview-nav-right" v-if="preview.isHorizontalNavEnable" v-show="!preview.loading" @click="nextAction">
      <span class="lg-preview-nav-arrow"></span>
    </div>
  </div>
</transition>
  ...
  methods: {
    leave(e) {
      if ((this.preview.show)
        && (e.target.className.indexOf('lg-preview-nav') != 0)
    ...
    changeDirection() {
      if (!this.preview.isRotateEnable) {
        this.close()
        return false;
      }
...
.lg-preview-nav-left:hover,
.lg-preview-nav-right:hover {
  background: rgba(255, 255, 255, .2);
}
.lg-preview-nav-left:hover .lg-preview-nav-arrow,
.lg-preview-nav-right:hover .lg-preview-nav-arrow {
  border-color: #000;
  border-width: 4px;
}
